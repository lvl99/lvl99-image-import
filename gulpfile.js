...
// JavaScript bundlers
var app = new Bundler( './js', 'lvl99-image-import.dev.js', './js', 'lvl99-image-import.js' );
...
gulp.task('build', ['jsdependencies', 'less', 'app']);
...
gulp.task('less', function () {
  return gulp.src(['./css/lvl99-image-import.less'])
    .pipe( less() )
    .pipe( prefix({
...
gulp.task('compressjs', ['jsdependencies', 'app'], function () {
  return gulp.src(['./js/lvl99-image-import.js',
                   './js/dependencies.js'])
    .pipe( uglify() )
    .pipe( rename({
      extname: '.min.js'
...
gulp.task('compresscss', ['less'], function () {
  return gulp.src(['./css/lvl99-image-import.css',
...
gulp.task('watchfiles', function () {
  var watchcss    = gulp.watch( './css/**/*.less', ['less'] ),
      watchjs     = gulp.watch( './js/lib/**/*.js', ['jsdependencies'] );
});
